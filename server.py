...
class DraftAppHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_player_news(self):
        try:
            ...
            dst_names = ['Defense', 'Broncos', 'Steelers', 'Eagles', 'Ravens', '49ers', 'Vikings', 'Texans', 'Bills', 'Jets', 'Cowboys', 'Chiefs', 'Lions', 'Chargers', 'Packers', 'Browns', 'Bears', 'Seahawks', 'Buccaneers', 'Dolphins', 'Colts', 'Rams', 'Saints', 'Jaguars', 'Falcons', 'Patriots', 'Bengals', 'Titans', 'Commanders', 'Giants', 'Cardinals', 'Raiders', 'Panthers']
            if pos == 'DST' or any(dn in player_name for dn in dst_names):
                prompt = f"""
You are a real-time fantasy football defense/special teams analyst and high-stakes auction draft strategist.
Analyze the NFL Team Defense & Special Teams (DST) unit: {player_name} ({team} Defense).
...
SEARCH & EVALUATE:
Search specifically for the {player_name} DEFENSIVE unit: defensive coordinator scheme changes, pass-rush pressure rate / sack floor, secondary coverage health/injuries, turnover generation, return game touchdowns, and early season streaming schedule (Weeks 1-4).
...
Analyze the NFL starting kicker: {player_name} ({pos}, {team}).
...
SEARCH & EVALUATE:
Search for recent news, preseason competition, 50+ yard accuracy, offensive drive volume, indoor dome advantage, and injury health for kicker {player_name} on the {team}.
...
Analyze NFL player: {player_name} ({pos}, {team}).
